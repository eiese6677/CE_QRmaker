(function () {
  ...
  // overlay가 입력을 가로채도록 설정 (그래야 요소 선택시 아래 페이지가 클릭에 반응하지 않음)
  overlay.style.pointerEvents = "auto";
  overlay.style.cursor = "crosshair";
  overlay.style.userSelect = "none";
  // overlay.style.webkitUserSelect = "none"; <- 안됨
  document.documentElement.appendChild(overlay);

  // 만든 직후 클릭 무시 (e.g., the popup button click)
  ...
  function updatePreviewForElement(el) {
    ...
    if (above) {
      previewEl.style.left =
        Math.min(window.innerWidth - prefWidth - 8, Math.max(8, rect.left)) +
        "px";
      ...
      previewEl.style.left =
        ...
        "px";
      previewEl.style.top =
        Math.min(window.innerHeight - 32, rect.bottom + 8) + "px";
    }
  }
  ...
  function onClick(e) {
    ...
    function highlightHTML(s) {
      ...
      return t
        .replace(/(&lt;\/?.*?&gt;)/g, '<span style="color:#9ad0ff">$1</span>')
        .replace(/(\b[a-z-]+)(=)/gi, '<span style="color:#ffd59a">$1</span>$2');
    }
    function highlightCSS(s) {
      const t = escapeHtml(s);
      return t.replace(
        /([.#]?[a-zA-Z0-9_\-\:\[\]\>\s,>]+)/g,
        '<span style="color:#9ad0ff">$1</span>'
      );
    }
    function highlightJS(s) {
      const t = escapeHtml(s);
      return t.replace(
        /(const|let|var|function|return|=>|if|else|new|document|window)/g,
        '<span style="color:#ffd59a">$1</span>'
      );
    }
    function makeBlock(title, code, lang) {
      ...
      header.className = "__ce_qr_block_header";
    ...
    copyAllBtn.className = "__ce_qr_copyall";
    ...
    copyAllBtn.addEventListener("click", (ev) => {
      ...
      const allText =
        (outer || "") + "\n\n" + (cssText || "") + "\n\n" + (jsText || "");
      tryCopy(allText);
      showTempMessage(copyAllBtn, "복사됨");
    });
    ...
    varPanel.appendChild(idLabel);
    varPanel.appendChild(idInput);
    varPanel.appendChild(classLabel);
    varPanel.appendChild(classInput);
    varPanel.appendChild(textLabel);
    varPanel.appendChild(textInput);

    // CSS custom properties editor
    const varsLabel = document.createElement("div");
    varsLabel.textContent = "CSS 변수 (name, value)";
      ...
      if (el.style) {
        for (let i = 0; i < el.style.length; i++) {
          ...
          if (prop && prop.startsWith("--"))
            addVarRow(prop, el.style.getPropertyValue(prop));
        }
      }
    } catch (e) {}
    varPanel.appendChild(varsWrap);
    const addVarBtn = document.createElement("button");
    addVarBtn.textContent = "변수 추가";
    addVarBtn.addEventListener("click", (ev) => {
      ev.stopPropagation();
      ev.preventDefault();
    ...
    propsLabel.textContent = "일반 CSS 속성 (name, value)";
    varPanel.appendChild(propsLabel);
    ...
    function addPropRow(name, value) {
      ...
      colorPicker.style.marginLeft = "6px";
      ...
      function parseToHex(input) {
        try {
          ...
          if (hslm) {
            ...
            let a = parts.length > 3 ? parseFloat(parts[3]) : 1;
            if (String(parts[1]).endsWith("%")) sPer = sPer / 100;
            else sPer = sPer;
            if (String(parts[2]).endsWith("%")) lPer = lPer / 100;
            else lPer = lPer;
            h = (((h % 360) + 360) % 360) / 360;
            const sN = sPer,
              lN = lPer;
      ...
      k.addEventListener("input", () => {
        try {
          const val = v.value;
          if (k.value) el.style.setProperty(k.value, val);
          if (isColorProp(k.value)) {
            try {
              colorPicker.style.display = "inline-block";
              const comp =
                window.getComputedStyle(el).getPropertyValue(k.value) ||
                v.value;
              const hex = parseToHex(comp);
      ...
      v.addEventListener("input", () => {
        try {
          const key = k.value;
          if (key) el.style.setProperty(key, v.value);
          if (isColorProp(key)) {
            try {
              // try to set colorPicker to the value if parseable
              const hx = parseToHex(v.value);
      ...
      colorPicker.addEventListener("input", () => {
        try {
          const key = k.value;
          if (key) {
            el.style.setProperty(key, colorPicker.value);
            v.value = colorPicker.value;
          }
        } catch (e) {}
      });
    }
    ...
    try {
      if (el.style) {
        if (el.style.color) addPropRow("color", el.style.color);
        if (el.style.backgroundColor)
          addPropRow("background-color", el.style.backgroundColor);
      }
    } catch (e) {}
    varPanel.appendChild(propsWrap);
    const addPropBtn = document.createElement("button");
    addPropBtn.textContent = "속성 추가";
    addPropBtn.addEventListener("click", (ev) => {
      ev.stopPropagation();
      ev.preventDefault();
      addPropRow("property", "value");
    });
    ...
    idInput.addEventListener("input", () => {
      try {
        el.id = idInput.value.trim();
      } catch (e) {}
    });
    classInput.addEventListener("input", () => {
      try {
        el.className = classInput.value.trim();
      } catch (e) {}
    });
    ...
    resetBtn.addEventListener("click", (ev) => {
      ...
      try {
        ...
        if (originalSnapshot.styleCssText)
          el.setAttribute("style", originalSnapshot.styleCssText);
    ...
    undoBtn.addEventListener("click", (ev) => {
      ...
      try {
        if (history.length > 1) {
          ...
          if (s.styleCssText) el.setAttribute("style", s.styleCssText);
          else el.removeAttribute("style");
          idInput.value = s.id;
          classInput.value = s.className;
          textInput.value = s.textContent;
          showTempMessage(undoBtn, "되돌려짐");
        }
      } catch (e) {}
    });
    ...
    (function makeDraggable(elm) {
      ...
      function onPointerDown(ev) {
        ...
        if (
          ...
          (ev.target.tagName === "BUTTON" ||
            ev.target.closest(".__ce_qr_buttons"))
        )
          return;
        isDown = true;
        startX = ev.clientX;
      ...
      function onPointerMove(ev) {
        ...
        elm.style.left =
          Math.min(window.innerWidth - 20, Math.max(8, origX + dx)) + "px";
        elm.style.top =
          Math.min(window.innerHeight - 20, Math.max(8, origY + dy)) + "px";
      }
